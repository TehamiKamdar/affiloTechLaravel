...
trait PerformanceGraph
{
    ...
    public function earningOverview($user, $earningOverviews)
    {
        ...
        return Cache::remember("earningOverview{$userID}{$websiteID}", 60 * 60, function () use($user, $earningOverviews) {
            ...
            if($user->getAllowed())
            {
                $earningOverviewsList['paid_status'] = $payment->where("status", PaymentHistory::PAID)->fetchPublisher($user)->sum("commission_amount");
                $earningOverviewsList['peending_status'] = $payment->where("status", PaymentHistory::PENDING)->fetchPublisher($user)->sum("commission_amount");
            }
            else
            {
