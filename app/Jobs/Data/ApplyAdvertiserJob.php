...
{
   use Dispatchable, Queueable, InteractsWithQueue, SerializesModels, GenerateLink, Approval, JoinedAdvertiserHold, JoinedAdvertiserReject, Reject;
        ...
        try {
            ...
            $trackURL = $shortURL = $longURL = null;
                $linkGenerate = AdvertiserPublisher::GENERATE_LINK_EMPTY;
                   \Log::channel('broadcast')->info(AdvertiserPublisher::STATUS_ACTIVE);
               \Log::channel('broadcast')->info('this is advertiser status');
                if($this->data["status"] == AdvertiserPublisher::STATUS_ACTIVE)
                {
                    ...
                    $linkGenerate = $generate['link_generate'];
                     \Log::channel('broadcast')->info($generate);
               \Log::channel('broadcast')->info('this is generate');
                ...
                $advertiserPublisher->update([
                    'approver_id' => 'b88a023a-4503-44d7-8e79-5fc5741fc093',
                 ...
                 Tracking::updateOrCreate([
                            ...
                            "click_through_url" => $advertiserPublisher->click_through_url,
                             'tracking_url_short' => $shortURL,
                             'tracking_url_long' => $longURL,
                              'tracking_url' => $trackURL,
                        ]);
                        
                     \Log::channel('broadcast')->info($advertiserPublisher);
               \Log::channel('broadcast')->info('this is advertiser publisher update');
